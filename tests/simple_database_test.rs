...
#[tokio::test]
async fn test_create_connection_pool_basic() {
...
#[tokio::test]
async fn test_create_connection_pool_with_env_vars() {
    let database_url = get_test_database_url().await;
...
async fn test_create_connection_pool_invalid_env_values() {
    ...
    let pool = create_connection_pool(&database_url).await;
    // Should succeed because invalid values fall back to defaults
    assert!(pool.is_ok());
...
#[tokio::test]
async fn test_database_connection_basic() {
    let database_url = get_test_database_url().await;
...
#[tokio::test]
async fn test_database_connection_queries() {
    let database_url = get_test_database_url().await;
    let pool = create_connection_pool(&database_url).await.unwrap();
...
async fn test_update_db_pool_metrics() {
    ...
    // Test with some connections in use
