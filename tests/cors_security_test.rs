...
async fn test_cors_preflight_disallowed_origin() {
    ...
    assert!(resp.status().is_client_error() || !resp.headers().contains_key("access-control-allow-origin"));
}
...
async fn test_cors_security_headers_validation() {
    ...
    if headers.contains_key("access-control-max-age") {
        let max_age = headers.get("access-control-max-age").unwrap().to_str().unwrap();
...
async fn test_cors_credentials_configuration() {
    ...
    if headers.contains_key("access-control-allow-credentials") {
        let credentials = headers.get("access-control-allow-credentials").unwrap().to_str().unwrap();
...
async fn test_cors_multiple_origins() {
    let cors = create_test_cors("https://app1.com,https://app2.com,http://localhost:3000", false, 3600);
    ...
    let allowed_origins = ["https://app1.com", "https://app2.com", "http://localhost:3000"];
