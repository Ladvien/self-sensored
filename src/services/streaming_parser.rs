use actix_web::{dev::Payload, error::ErrorBadRequest, web::Bytes, HttpRequest, Result as ActixResult, Error as ActixError};
...
impl StreamingJsonParser {
        ...
        while let Some(chunk_result) = stream.next().await {
            ...
            if self.total_size + chunk.len() > self.max_size {
                error!(
                    ...
                    self.total_size, chunk.len(), self.max_size
                );
            ...
            if self.total_size > 10 * 1024 * 1024 { // > 10MB
                if self.total_size % (5 * 1024 * 1024) == 0 { // Every 5MB
                    info!("Streaming progress: {} MB received", self.total_size / (1024 * 1024));
                }
            }
        }
    ...
    fn parse_complete_json<T: DeserializeOwned>(&self) -> ActixResult<T> {
        ...
        if let Err(validation_error) = self.validate_json_structure() {
            ...
            return Err(ErrorBadRequest(format!("Malformed JSON: {}", validation_error)));
        }
...
pub async fn parse_large_json_payload<T>(
    req: &HttpRequest,
    payload: Payload,
) -> ActixResult<T>
where
    T: DeserializeOwned,
{
    ...
    debug!("Processing JSON payload with content-length: {:?}", content_length);
