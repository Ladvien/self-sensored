...
mod timeout_tests {
    ...
    fn test_timeout_environment_variable_parsing() {
        ...
        assert_eq!(default_timeout, 60, "Default timeout should be 60 seconds for DoS protection");
        ...
        assert_eq!(connection_timeout, 30, "Connection timeout should be 30 seconds");
        ...
        assert_eq!(keep_alive_timeout, 15, "Keep-alive timeout should be 15 seconds");
    ...
    fn test_cloudflare_timeout_configuration() {
        ...
        assert!(our_timeout > 30, "Timeout should be reasonable for health data processing");
    ...
    fn test_payload_size_limits() {
        ...
        assert_eq!(max_payload_size_mb, 50, "Max payload should be 50MB for DoS protection");
        assert!(max_payload_size_mb < 100, "Payload size should be reasonable for health data");
        assert!(max_payload_size_mb > 10, "Payload size should accommodate legitimate health data");
        
        println!("✅ Payload size limits test completed");
        println!("   - Max payload size: {}MB (DoS-protected)", max_payload_size_mb);
    }
    ...
    fn test_dos_protection_timeouts() {
        ...
        assert!(keep_alive_timeout < connection_timeout, "Keep-alive should be shorter than connection timeout");
        assert!(connection_timeout < request_timeout, "Connection timeout should be shorter than request timeout");
        
        // Ensure timeouts are reasonable for health data but protect against DoS
        assert!(request_timeout >= 30, "Request timeout should allow reasonable health data processing");
        assert!(request_timeout <= 90, "Request timeout should prevent resource exhaustion");
