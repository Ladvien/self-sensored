...
/// Test dual-write functionality for activity metrics
#[tokio::test]
async fn test_dual_write_functionality_comprehensive() {
    let pool = get_test_pool().await;
    let redis_client = get_test_redis_client();
    let (user_id, api_key) = setup_test_user_and_key(&pool, "dual_write_api@example.com").await;

    let app = create_api_test_app(pool.clone(), redis_client).await;

    // Enable dual-write mode
    std::env::set_var("DUAL_WRITE_ACTIVITY_METRICS", "true");

    println!("🚀 Testing dual-write functionality comprehensively...");

    // Test 1: Standard activity metrics
    let activity_v1_payload = json!({
        "data": {
            "metrics": [
                {
                    "type": "Activity",
    ...
    let req = test::TestRequest::post()
        ...
        .insert_header(("Authorization", format!("Bearer {}", api_key)))
        .insert_header(("content-type", "application/json"))
        .set_json(&activity_v1_payload)
        .to_request();

    let resp = test::call_service(&app, req).await;
    assert!(resp.status().is_success());

    // Test 2: Activity metrics v2 format
    let activity_v2_payload = json!({
        "data": {
            "activity_metrics_v2": [
                {
                    "recorded_at": "2024-01-15T14:00:00Z",
                    "step_count": 15000,
                    "distance_walking_running_meters": 10500.0,
                    "distance_cycling_meters": 2000.0,
                    "active_energy_burned_kcal": 550.0,
                    "basal_energy_burned_kcal": 1800.0,
                    "exercise_time_minutes": 95,
                    "stand_time_minutes": 480,
                    "move_time_minutes": 300,
                    "stand_hour_achieved": true,
                    "flights_climbed": 18,
                    "aggregation_period": "daily",
                    "source": "Dual Write Test V2"
                }
            ]
        }
    });
    ...
    let req = test::TestRequest::post()
        ...
        .insert_header(("Authorization", format!("Bearer {}", api_key)))
        .insert_header(("content-type", "application/json"))
        .set_json(&activity_v2_payload)
        .to_request();

    let resp = test::call_service(&app, req).await;
    assert!(resp.status().is_success());

    // Test 3: Mixed payload with both formats
    let mixed_activity_payload = json!({
        "data": {
            "metrics": [
                {
                    "type": "Activity",
                    ...
                    "source": "Mixed Test V1"
                }
            ],
            "activity_metrics_v2": [
                {
                    "recorded_at": "2024-01-16T16:00:00Z",
                    "step_count": 8000,
                    "distance_walking_running_meters": 6000.0,
                    "active_energy_burned_kcal": 300.0,
                    "exercise_time_minutes": 60,
                    "source": "Mixed Test V2"
                }
            ]
        }
    });
    ...
    let req = test::TestRequest::post()
        ...
        .insert_header(("Authorization", format!("Bearer {}", api_key)))
        .insert_header(("content-type", "application/json"))
        .set_json(&mixed_activity_payload)
        .to_request();

    let resp = test::call_service(&app, req).await;
    assert!(resp.status().is_success());

    // Verify dual-write results
    let v1_count = sqlx::query!(
    ...
    let v2_count = sqlx::query!(
        "SELECT COUNT(*) as count FROM activity_metrics_v2 WHERE user_id = $1",
        user_id
    )
    .fetch_one(&pool)
    .await
    ...
    println!("✅ Dual-Write Results:");
    println!("   📊 Activity V1 records: {}", v1_count);
    println!("   📈 Activity V2 records: {}", v2_count);

    // Both tables should have records
    assert!(v1_count >= 2, "Should have records in activity_metrics");
    assert!(v2_count >= 2, "Should have records in activity_metrics_v2");

    // Verify field mapping consistency
    verify_dual_write_consistency(&pool, user_id).await;

    // Disable dual-write mode
    std::env::set_var("DUAL_WRITE_ACTIVITY_METRICS", "false");
    cleanup_test_data(&pool, user_id).await;
}
...
async fn verify_dual_write_consistency(pool: &PgPool, user_id: Uuid) {
    // Get a sample record from each table to verify field mapping
    let v1_record = sqlx::query!(
        "SELECT steps, distance_meters, calories_burned, active_minutes, flights_climbed 
         FROM activity_metrics WHERE user_id = $1 LIMIT 1",
        user_id
    )
    .fetch_optional(pool)
    .await
    ...
    let v2_record = sqlx::query!(
        "SELECT step_count, distance_walking_running_meters, active_energy_burned_kcal, 
                exercise_time_minutes, flights_climbed 
         FROM activity_metrics_v2 WHERE user_id = $1 LIMIT 1",
        user_id
    )
    .fetch_optional(pool)
    .await
    ...
    if let (Some(v1), Some(v2)) = (v1_record, v2_record) {
        // Verify field mappings are consistent where they should be
        assert_eq!(v1.flights_climbed, v2.flights_climbed, "Flights climbed should match");
        // Note: Other fields may not match exactly due to dual-write timing and different data sources
    }
}
