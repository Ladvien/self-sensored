...
# MQTT Setup Script for Manjaro Linux (192.168.1.110)
# Run with: sudo bash setup_mqtt_manjaro.sh

set -e

echo "=== MQTT Setup for Health Export API (Manjaro) ==="
echo "Starting at $(date)"

# Step 1: Install Mosquitto (includes broker and client tools)
echo ""
echo "Step 1: Installing Mosquitto MQTT Broker and client tools..."
...
# Step 7: Configure firewall (using ufw or iptables)
echo ""
...
if command -v ufw &> /dev/null; then
    ...
    # Check for firewalld (common on Manjaro)
    if command -v firewall-cmd &> /dev/null; then
        if systemctl is-active --quiet firewalld; then
            echo "Adding firewalld rule for MQTT WebSocket..."
            firewall-cmd --permanent --add-rich-rule='rule family="ipv4" source address="192.168.1.102" port port="9001" protocol="tcp" accept'
            firewall-cmd --reload
        else
        ...
        # Save iptables rules for Manjaro
